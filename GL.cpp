...
void viewport(int x, int y, int w, int h, float depth) { //depth = f-n;
...
void lookAt(Vec3f camera, Vec3f target, Vec3f up) {
   ...
   ModelView = Matrix::identity(); //Identity matrix causes divide by 0 or something
   for (int i=0; i<3; i++) {
        ModelView[0][i] = x[i];
        ModelView[1][i] = y[i];
        ModelView[2][i] = z[i];
